...
diary Redundancy_Report_2010-05-19_g_Criterion_5.txt
...
drawnow
      ...
      if (min(length(ti),length(tj)) - matches <= 4) || (pro > p),
        ...
        if n(i,2) > 1300,
          in = lower(t{i,8});
          jn = lower(t{j,8});
          if length(in) > 1 && length(jn) > 1,
            [namematches,na,nb,nss,ntt] = dNeedlemanWunsch(in,jn); 
            namematches = sum(in(na) == jn(nb));
          else
            namematches = 0;
          end
          namelength = min(length(in), length(jn));
          namelength = max(namelength,1);

                                              % check bio source for ribosomes
                                              % some have similar sequences
          if namelength > 5 && namematches < namelength / 2,
            fprintf('    %s and %s are different organisms but have %7.4f%% sequence identity\n', in, jn, pro*100);
      ...
      if prop(i,j) > 0.8,                 % if even a little sequence agreement
...
closeseq1 = Close;                 % store for later as well
...
Discrepancies = sparse(zeros(F,F));
