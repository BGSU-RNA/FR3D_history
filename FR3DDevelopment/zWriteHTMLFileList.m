...
Reports = [[1 Inf]; [2 Inf]; [2 20]; [2 4]; [2 3.5]; [2 3]; [2 2.5]; [2 2]; [2 1.5]];
...
case 1, 
  ...
  fprintf(fid,'<a href="All_structures_%s_list.pdb">Click here for the list file to use with FR3D</a> (Save in FR3D/PDBFiles.)<br>', NRDate);
...
case 2,
  ...
  fid = fopen(['Web\AnalyzedStructures\Nonredundant_' MRText '.html'],'w');
  fprintf(fid,'<html>');
  fprintf(fid,'<head>\n');
  if MaxRes < Inf,
    fprintf(fid,'<title>Non-redundant list of PDB structures from X-ray crystallography with resolution %7.1fA or better</title>\n', MaxRes);
  else
    fprintf(fid,'<title>Non-redundant list of PDB structures of all resolutions</title>\n');
  end
  fprintf(fid,'</head>\n');
  fprintf(fid,'<body>\n');
  if MaxRes < Inf,
    ...
    fprintf(fid,'<h3>Non-redundant list of %d PDB structures of all resolutions and containing at least one RNA basepair</h3>\n', length(i));
  end
  fprintf(fid,'They are sorted by the length of the longest unique chain, longest to shortest.<br>\n');
  fprintf(fid,'<a href="Nonredundant_%s_%s_list.pdb">Click here for the list file to use with FR3D</a> (Save in FR3D/PDBFiles.)<br>', MRText, NRDate);
...
switch Format
case 1,
  fid2 = fopen(['Web\AnalyzedStructures\All_structures_' NRDate '_list.pdb'],'w');
  for i = 1:length(tt(:,1)),
...
case 2,
  ...
  fid2 = fopen(['Web\AnalyzedStructures\Nonredundant_' MRText '_' NRDate '_list.pdb'],'w');
  for i = 1:length(tt(:,1)),
    fprintf(fid2,'%s\n', tt{i,1});                % write to the list
  end
  fclose(fid2);
