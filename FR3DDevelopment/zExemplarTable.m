...
%       text(0.3*a(1)+0.7*a(2),0.8*a(3)+0.2*a(4),['Count: ' num2str(E.Count)]);
       ...
       if F.NT(1).Code == 4 && F.NT(2).Code == 4,
         V = ViewParam;
         V.Sugar = 0;
         V.LabelBases = 0;
         V.GlycoAtomSize = 0;
       ...
       plot([NT2Fit(1,1) NT2Fit(h2,1)],[NT2Fit(1,2) NT2Fit(h2,2)],'k', 'LineWidth', 2);
...
%       plot([NT1Fit(1,1) NT1Sug(1,1)],[NT1Fit(1,2) NT1Sug(1,2)],'k', 'LineWidth', 2);
       ...
       plot(NT2Fit(h2,1), NT2Fit(h2,2), '.k', 'MarkerSize', 16);
       hold on
