...
function [Discrepancy, Candidates] = xRankCandidatesIDI(File,Model,Cand,Verbose)
...
%  A = xDiscrepancyFast(Model,File(Cand(i,N+1)).NT(Cand(i,[1:N])));
  ...
  A = zIsoDiscrepancy(Model.NT(1),Model.NT(2),File(Cand(i,N+1)).NT(Cand(i,1)),File(Cand(i,N+1)).NT(Cand(i,2)));
