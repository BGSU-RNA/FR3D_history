% zDisplayNTCenters is used for the paper on motif searching
...
for j=1:length(Indices),                 % Display centers and lines
  for k=(j+1):length(Indices),
    ...
    AA = (A - ones(2,1)*S) * R;
    plot3(AA(:,1), AA(:,2), AA(:,3),'Color',VP.Color,'LineWidth',2,'LineStyle','-');
    plot3(AA(:,1), AA(:,2), AA(:,3),'.','Color',VP.Color,'MarkerSize',18);
    hold on
  end
end
