% zDisplayNTCentersAddOne is used for the paper on motif searching
% it displays the last nucleotide with red lines
...
for j=1:length(Indices),                 % Display centers and lines
  for k=(j+1):length(Indices),
    A  = [File.NT(Indices(j)).Center; File.NT(Indices(k)).Center];
    AA = (A - ones(2,1)*S) * R;
    if (k==length(Indices)),
      plot3(AA(:,1), AA(:,2), AA(:,3),'k','LineWidth',2,'LineStyle','-');
      plot3(AA(2,1), AA(2,2), AA(2,3),'k.','MarkerSize',18);
    else
      plot3(AA(:,1), AA(:,2), AA(:,3),'r','LineWidth',2,'LineStyle','-');
      plot3(AA(:,1), AA(:,2), AA(:,3),'r.','MarkerSize',18);
    end
    hold on
  end
end
