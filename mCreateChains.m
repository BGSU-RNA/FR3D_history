...
% mDetermineChains %this is a program that extracts chain information from the already read query PDB file
for i=1:12
    h=findobj('Tag',strcat('ChainPopup',num2str(i)));
    delete(h);
end
for i=1:length(NT)
    Ch=mGetChainsOfNTnumber(File(SIndex),NT(i));%So now Ch is individual for each numcleotide number
%     if length(Ch)>1
